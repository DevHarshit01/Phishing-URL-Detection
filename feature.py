...
class FeatureExtraction:
    ...
    def WebsiteTraffic(self):
        try:
            alexa_url = os.environ.get("ALEXA_URL", "http://data.alexa.com/data?cli=10&dat=s&url=")
            rank = BeautifulSoup(urllib.request.urlopen(alexa_url + self.url).read(), "xml").find("REACH")['RANK']
            if (int(rank) < 100000):
                return 1
            return 0
        except :
    ...
    def PageRank(self):
        try:
            pagerank_url = os.environ.get("PAGERANK_URL", "https://www.checkpagerank.net/index.php")
            prank_checker_response = requests.post(pagerank_url, {"name": self.domain})
    ...
    def GoogleIndex(self):
        try:
            limit = int(os.environ.get("GOOGLE_SEARCH_LIMIT", 5))
            site = search(self.url, limit)
            if site:
                return 1
            else:
                return -1
